...
const Root = styled('div')<SystemStyleObject<Theme> & FusePageCardedProps>(({ theme, ...props }) => ({
	display: 'flex',
	...
	return (
		<>
			...
			<Root
				...
				ref={rootRef}
				scroll={scroll as 'content' | 'page' | 'normal'}
				leftSidebarWidth={leftSidebarWidth as number}
				rightSidebarWidth={rightSidebarWidth as number}
			>
				{header && <FusePageCardedHeader header={header} />}

				<div className={`relative z-10 flex h-full flex-auto flex-col overflow-hidden ${className}`}>
...
export default memo<React.FC<FusePageCardedProps>>(styled(FusePageCarded)``);
